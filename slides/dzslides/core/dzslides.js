...
Dz.onclick = function(aEvent) {
  ...
  Dz.onmousewheel = function(aEvent) {
    if (!aEvent.wheelDeltaY) return;

    if (aEvent.wheelDeltaY < 0){
      aEvent.preventDefault();
      this.forward();

    }else if (aEvent.wheelDeltaY > 0){
      aEvent.preventDefault();
      this.back();
...
function init() {
  ...
  window.onmousewheel = Dz.onmousewheel.bind(Dz);
}
